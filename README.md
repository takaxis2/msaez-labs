...
- 추가 설치한 Software는 Gitpod 접속과 동시에 실행되는 .gitpod.yml에 기술하여 설치 가능하다.
- 아래 4개의 Lab에 필요한 추가 Software가 .gitpod.yml에 정의되어 설치되도록 설정되어 있다. (init.sh)

- 1. CLI기반 Web Client - Httpie (curl, POSTMAN 대용) 
```
sudo apt-get update
...
- 2. kubernetes utilities (kubectl)
```
...
## EDA 기반 마이크로서비스 통신에 필요한 Kafka 설치
